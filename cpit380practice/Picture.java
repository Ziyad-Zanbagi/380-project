...
public class Picture extends SimplePicture {
    ...
    public void MedianFilter(Picture p) {

        int x = p.getWidth();
        int y = p.getHeight();
        Picture copy = new Picture(x, y);
        copy.copy(p, 0, 0, x, y, 0, 0);
        int R[] = new int[9];
        int G[] = new int[9];
        int B[] = new int[9];
        for (int i = 1; i <= y - 2; i++) {
            for (int j = 1; j <= x - 2; j++) {
                ...
                for (int k = -1; k <= 1; k++) {
                    for (int l = -1; l <= 1; l++) {
                        R[a] = copy.getPixel(j + l, i + k).getRed();
                        G[a] = copy.getPixel(j + l, i + k).getGreen();
                        B[a] = copy.getPixel(j + l, i + k).getBlue();
                        a++;
                    }
                }
                Arrays.sort(R);
                Arrays.sort(G);
                Arrays.sort(B);
                p.getPixel(j, i).setColor(new Color(R[3], G[3], B[3]));
            }
        }
    }

    public void MinFilter(Picture p) {
        ...
        for (int i = 1; i <= y - 2; i++) {
            for (int j = 1; j <= x - 2; j++) {
                ...
                Arrays.sort(G);
                Arrays.sort(B);
                p.getPixel(j, i).setColor(new Color(R[0], G[0], B[0]));
            }
        }
    }

    public void MaxFilter(Picture p) {
        ...
        for (int i = 1; i <= y - 2; i++) {
            for (int j = 1; j <= x - 2; j++) {
                ...
                Arrays.sort(G);
                Arrays.sort(B);
                p.getPixel(j, i).setColor(new Color(R[8], G[8], B[8]));
            }
        }
    ...
    public void mirrorTempleMy() {
        ...
        // loop through the rows
        for (int y = 127; y < 400; y++) {
            // loop from 13 to just before the mirror point
            for (int x = 213; x < mirrorPoint; x++) {
                System.out.println("Copying color from "
                        + x + " to "
                        + (mirrorPoint + (mirrorPoint - x)));
                leftPixel = getPixel(x, y);
    ...
    public void convert2Binary(double amount) {
        this.grayscale();
     ...
     * current picture
     */
    public void overlapPictures(Picture p1, int startOverlap) {
        Picture p2
    ...
    public Picture collage() {
        Picture collage = new Picture(this);
        Picture p1 = new Picture(this);
        p1 = p1.scaleDown(3);
        Picture p2 = new Picture(p1);
        p2.keepRed();
        Picture p3 = new Picture(p1);
        p3.keepGreen();
        Picture p4 = new Picture(p1);
        p4.keepBlue();
        for (int x = 0; x < p2.getWidth(); x++) {
            for (int y = collage.getHeight() - 1 - p2.getHeight(), ty = 0; y < collage.getHeight() - 1; y++, ty++) {
                collage.getPixel(x, y).setColor(p2.getPixel(x, ty).getColor());

            }
        }
        for (int x = p2.getWidth(), tx = 0; x < p2.getWidth() + p2.getWidth(); x++, tx++) {
            for (int y = collage.getHeight() - 1 - p2.getHeight(), ty = 0; y < collage.getHeight() - 1; y++, ty++) {
                collage.getPixel(x, y).setColor(p3.getPixel(tx, ty).getColor());

            }
        }
        for (int x = p2.getWidth() + p2.getWidth(), tx = 0; x < collage.getWidth(); x++, tx++) {
            for (int y = collage.getHeight() - 1 - p2.getHeight(), ty = 0; y < collage.getHeight() - 1; y++, ty++) {
                collage.getPixel(x, y).setColor(p4.getPixel(tx, ty).getColor());

            }
        }
        collage = collage.mirrorAllHorizontal();
        collage = collage.mirrorAllVertical();
        return collage;
    ...
    public void copyKatie() {
        ...
        //    FileChooser.getMediaPath("C:\\Users\\1406592\\Desktop\\tools-380\\intro-prog-java\\MediaSources\\KatieFancy.jpg");
        Picture sourceFile = new Picture(FileChooser.pickAFile());
    ...
    public void copyKatiesFace(Picture pictObj, int X, int Y) {
        ...
        for (int sourceX = sx, targetX = 100;
                sourceX < 250; sourceX++, targetX++) {
            // loop through the rows
            for (int sourceY = sy, targetY = 100;
                    sourceY < 165; sourceY++, targetY++) {
                // set the target pixel color to the source pixel color
                sourcePixel = sourcePicture.getPixel(sourceX, sourceY);
                targetPixel = this.getPixel(targetX, targetY);
                targetPixel.setColor(sourcePixel.getColor());
    ...
    public void copyFlowers() {
        ...
        Picture sourceFile1 = new Picture(FileChooser.pickAFile());
        Picture sourcePicture1 = new Picture(sourceFile1);
        ...
        int flower1Height = sourceFile1.getHeight();
        int flower2Height = sourceFile.getHeight();

        /* copy the first flower picture to 5 pixels from the bottom
     * left corner of the current picture
         */
        for (int sourceX = 0, targetX = 0;
                sourceX < sourceFile1.getWidth();
                sourceX++, targetX++) {
            for (int sourceY = 0,
                    targetY = this.getHeight() - flower1Height - 5;
                    sourceY < sourceFile1.getHeight();
                    sourceY++, targetY++) {
                sourcePixel = sourceFile1.getPixel(sourceX, sourceY);
                targetPixel = this.getPixel(targetX, targetY);
                targetPixel.setColor(sourcePixel.getColor());
            }
        }
...
//             300,0);
    ...
    public void blendPictures() {
        ...
        Picture katiePicture = new Picture(FileChooser.pickAFile());
        Picture sourcePicture1 = new Picture(katiePicture);

        Picture jennyPicture = new Picture(FileChooser.pickAFile());
        Picture sourcePicture = new Picture(jennyPicture);
    ...
    public void copyKatieLeftRotation() {
        ...
        Picture sourcePicture1 = new Picture(sourcePicture);
        Pixel sourcePixel = null;
        Pixel targetPixel = null;
    ...
    public void copyJakitaSmaller() {
        Picture jakitaPicture = new Picture(FileChooser.pickAFile());
        Picture sourcePicture1 = new Picture(jakitaPicture);
        Pixel sourcePixel = null;
        Pixel targetPixel = null;
    ...
    public Picture flipVertical() {
        ...
        for (int x = 0; x < this.getWidth(); x++) {
            for (int y = 0; y < this.getHeight(); y++) {
                copy.getPixel(x, this.getHeight() - 1 - y).setColor(this.getPixel(x, y).getColor());
            }
        }
        return copy;
    }
    ...
    public void copyFlowerLarger() {
        ...
        Picture sourcePicture1 = new Picture(flowerPicture);
        Pixel sourcePixel = null;
        Pixel targetPixel = null;
       ...
       startX, startY, and endX and endY */
        for (int x = startX; x < endX; x++) {
            for (int y = startY; y < endY; y++) {
                ...
                if (pixel.colorDistance(Color.red) < 140) {
    ...
    public Picture scale(int numTimes) {
        Picture targetPicture;
        if (numTimes > 50) {
            targetPicture
                    = new Picture(this.getWidth() * numTimes,
            ...
            targetPicture = new Picture(this.getWidth() / numTimes + 1, this.getHeight() / numTimes + 1);
            Pixel sourcePixel = null;
            Pixel targetPixel = null;
    ...
    public void swapBackground(Picture oldBackground,
         ...
         * then swap in the new background pixel
                 */
                if (currPixel.colorDistance(oldPixel.getColor()) < 200.0) {
    ...
    public void trafficlight(Picture oldBackground
    ) {
         ...
         * then swap in the new background pixel
                 */
                if (currPixel.colorDistance(oldPixel.getColor()) != currPixel.getColor())  {
                    if (oldPixel != currPixel ) {
                        System.out.println("violation");
                    }
                    else {
                        System.out.println("not violation");
                    }
                }
            }
        }
     ...
     * pixel color
     */
    public void trafficstopline(Picture oldBackground,
            Picture newBackground) {
         ...
         * then swap in the new background pixel
                 */
                if (currPixel.colorDistance(oldPixel.getColor()) != newPixel.getBlue() + oldPixel.getRed()) {
    ...
    public static void main(String[] args) {
        Picture p = new Picture(FileChooser.pickAFile());
        Picture a = new Picture(FileChooser.pickAFile());
        p.trafficstopline(p, a);
        //a.chromkeyBlue(p);
        //   p.mirrorTemple();

        //  p.copyKatiesFace(p, 140, 125);
        // p.removeRedEye(46, 121, 441, 197, Color.BLACK);
        p.show();
    }
